...
export const JulesSessionManagement: TextFieldClientComponent = (props) => {
  ...
  const julesSessionId = useFormFields(([fields]) => fields.julesSessionId)?.value as Feedback['julesSessionId'];
  const developerPrompt = useFormFields(([fields]) => fields.developerPrompt)?.value as Feedback['developerPrompt'];
  ...
  const handleStartSession = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const res = await response.json().catch(() => ({ error: 'Failed to parse error response.' }));
