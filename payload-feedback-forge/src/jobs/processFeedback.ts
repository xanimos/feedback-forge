...
import { getFeedbackProcessor } from '../flows/feedbackProcessor.js'

export const getProcessFeedbackJob = (options: PayloadFeedbackForgeConfig): TaskConfig<'feedbackForge_processFeedback'> => {
  return {
    ...
    handler: async ({ input, job: _, req }) => {
      ...
      try {
        ...
        if (feedbackDoc) {
          const { breadcrumbs, feedback } = feedbackDoc

          // Get the configured Genkit flow
          const feedbackProcessor = getFeedbackProcessor(options)
